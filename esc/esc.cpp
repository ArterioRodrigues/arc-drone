...
boolean ESC::sendDShotPacket(uint16_t throttle1, uint16_t throttle2, uint16_t throttle3, uint16_t throttle4) {
    ...
    for (int m = 0; m < 4; m++) {
        uint16_t packet = (throttles[m] << 1);
        uint16_t checksum = ((packet >> 0) ^ (packet >> 4) ^ (packet >> 8)) & 0xF;
        packet = (packet << 4) | (checksum & 0xF);

        for (int i = 15; i >= 0; i--) {
            if (packet & (1 << i))
                items[m][15 - i] = this->_dShotBit1;
            else
                items[m][15 - i] = this->_dShotBit0;
        }
    ...
    esp_err_t error3 = rmt_write_items(this->_channel3, items[2], 17, true);
    esp_err_t error4 = rmt_write_items(this->_channel4, items[3], 17, true);

    if (error1 != ESP_OK || error2 != ESP_OK || error3 != ESP_OK || error4 != ESP_OK) {
        Serial.printf("RMT send error - CH1: %d, CH2: %d, CH3: %d, CH4: %d\n", error1, error2, error3, error4);
        return false;
    }
